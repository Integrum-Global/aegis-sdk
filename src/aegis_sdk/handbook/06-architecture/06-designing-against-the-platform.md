...
Start from what your code is _doing_, not from which module name looks likely.
...
There is a hard line between the system's _state_ and its _configuration_, and it
...
Everything in the _not reachable_ band is there because no route in this client
reaches it — and that cannot distinguish _the capability does not exist_ from
_this client does not expose it_. From where you stand those two are
...
consequential question about your system — _does a bound get consulted before the
effect, or after it?_ — is answered by whichever team configured the deployment,
...
The one readiness read you _can_ make is
`api:GET /api/v1/governance/envelope-coverage` — it answers not _"do bounds
exist"_ but _"would they stop anything"_ — and 06.4 covers how to read it
...
override from a kill switch. _Derived, not observed here:_ that opacity is
...
ask it. Build the habit of asking _which_ bound, rather than treating the first
...
**runtime-dependent**. The behaviour is guaranteed _on some runtimes and not on
others_, decided by deployment configuration you cannot see from here.
...
| Another organisation's artifact reads as _not found_   | **guaranteed**          | a deliberate refusal shape, so a 404 never confirms existence                                           |
...
| **A verdict landing _before_ the side effect**         | **runtime-dependent**   | only one runtime carries a genuine pre-execution tool gate, and it is not the shipped default           |
...
| **Objective status**                                   | **conventional**        | `sdk:aegis_sdk.ObjectiveStatus` is a _lossy_ projection of nine wire states onto six values             |
...
`pending` onto one value, and maps `decision` — _stopped, waiting for a person_ —
...
> permission to _try_, and handle the refusal at the write.
>
...
Whether the platform _additionally_ enforces a quota when you actually attempt
...
_Symptom:_ a control that refuses reliably in a demo and does not refuse in
...
_Diagnosis:_ enforcement here is layered, and the layers do not live in the same
...
establish _which_ layer produces it, and confirm that layer is on the path your
...
_Symptom:_ a run that stopped, a hold that was answered, and an external system
that had already been changed. Or an audit that reads clean on the record and
surprising in the world.
_Diagnosis:_ this is the most expensive misreading available on this platform. A
...
_Symptom:_ a governance claim you cannot support, discovered in the room where
you have to support it.
_Diagnosis:_ the shipped default runtime has no pre-execution tool gate, and the
platform says so out loud at boot rather than leaving it to be discovered. Of the
six runtimes, one carries the gate. Whether _your_ deployment is on it is a
...
_Symptom:_ a design that depends on no unclassified content leaving the process,
and content leaving unclassified.
_Diagnosis:_ the classification mechanism ships, is fail-closed inside a guarded
environment, and covers the LLM calls your own process makes — but in the
canonical default no environment is guarded and no classifier is installed, so it
is a no-op pass-through. Both halves are the deployment's to supply. It also
cannot cover a runtime that spawns a subprocess, by construction: 06.2 has that
boundary and it is the reason some designs must disable those runtimes rather
than police them.

**5. Building control flow on the typed status enums.**
_Symptom:_ work that is stopped on a human reads as running, so nothing
escalates and nothing reminds anyone. You find out when a request has been
sitting for a week.
_Diagnosis:_ read the wire status from a raw-dict route and keep your own state
machine. Treat the enums as filters for coarse queries.

**6. Treating a `403` as a scope problem.**
_Symptom:_ a refusal whose message names personas you do not have, on a route
...
_Diagnosis:_ on persona-gated routes the scopes are never consulted. Retry the
...
_Symptom:_ the approvals queue is empty, so the operator reports all clear,
while a pending decision and an escalated request sit unanswered for weeks.
_Diagnosis:_ there is no aggregating endpoint — each kind of waiting is its own
...
_Symptom:_ a decision nobody answered, and work that never completed. No error
anywhere.
_Diagnosis:_ the work holds a state; it does not occupy a queue. A decision's
...
_Symptom:_ a well-shaped method that answers `404` every time — or, worse, one
you wire into a retry path and never notice.
_Diagnosis:_ this client declares operations the server does not serve. Whether
...
_Symptom:_ a change that was approved and never took effect; a system where the
review says done and the state says otherwise.
_Diagnosis:_ approve and apply are separate steps on the change-request surface.
...
want.**
_Symptom:_ an upload accepted with an unexpected classification, or refused
because the request is not attached to a workspace.
_Diagnosis:_ the workspace comes from the request and there is no argument for
it. Choose the request you attach to; it chooses everything else.

**12. Reading a version gap as corruption.**
_Symptom:_ `api:GET /api/v1/artifacts/{}/versions` returns `1, 2, 4`.
_Diagnosis:_ each version is clearance-checked individually, so a version you may
...
was refused.
_Diagnosis:_ the clearest example is tool-agent invocation, where this client
raises _before_ sending anything if a tool call is malformed, precisely because
...
_Next: [Part 07 — Deployment architecture](../07-deployment-architecture/README.md)_
