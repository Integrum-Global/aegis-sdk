...
between them decide which one you reach for.
...
You do not administer the process. You administer what is _inside_ it.
...
The handbook ships _inside_ the package, at `aegis_sdk/handbook/`, so the
verification command in the introduction runs on your own machine against your
own copy.
...
form you read. Every path below is a link, and all of them resolve in your
checkout.
...
⚠ **"Agent brief", not "agent".** This platform's own word _agent_ means the thing
...
behind them. They take precedence, and the probe's output is the reading to
trust about your own deployment.

## What this book cites, and why

You build against a running Aegis over HTTP. This book therefore cites only
**surface you can reach** — an HTTP operation this client performs, written
`api:POST /api/v1/trust/establish`, or a symbol this package exports, written
`sdk:aegis_sdk.TrustChain`. It cites no file paths and no line numbers into the
platform's own source, because a coordinate into a tree you do not hold is a
coordinate you cannot follow.

Both forms resolve on your machine under `python -m aegis_sdk.handbook.check`,
so every operation and every symbol this book names can be confirmed against the
...
heavier one. The enforcement sits behind the API rather than in either client,
which is why the same call from either side lands on the same verdict.

---

_Next: [01.2 — The mental model](02-the-mental-model.md)_
