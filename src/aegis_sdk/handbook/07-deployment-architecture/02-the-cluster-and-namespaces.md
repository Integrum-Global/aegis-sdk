...
- **Not covered:** anything running *in* the cluster. Your workloads, their
...
namespace-scoped administrative path. *Design intent, not observable:* which of
those your deployment provides, and it is a question worth asking up front rather
...
**Autoscaling is the normal state, not an event.** *Design intent, not
observable:* that pools grow and shrink as demand moves is how the reference
shape is built rather than something visible from your integration. A consequence
worth internalising regardless: a pod that cannot be scheduled is not necessarily
a quota problem — it may be a node that has not come up yet, and the two have
different fixes and different durations.
...
Read it as: the pools are the top-level grouping, and namespaces live *inside*
...
*Design intent, not observable:* the number and naming of pools. The reference
shape is one system pool plus one or more workload pools; a specific deployment
may carry more, sized differently. Do not plan against a pool count — plan
against the rule that separates them.
...
| **Workload identity** | The identity provider, federated into the cluster | What a pod may authenticate *as*, and therefore what it may reach — [07.6](06-identity-and-secrets.md) |
...
*Design intent, not observable:* which of the enforcement mechanisms above your
deployment actually applies, and how strictly. Network policy in particular is
frequently present in the reference shape and variably enforced in practice.
This is a question for your operator, and it is one of the more important ones to
ask.
...
mechanisms, and *design intent, not observable* applies to the details:
...
**UNVERIFIED:** whether node affinity and taints are the *only* placement
controls in a given deployment, or whether additional admission policy is
applied. The reference shape uses the two above; a specific estate may add more,
and this document cannot see into it.

---

*Next: [07.3 — Images and the registry](03-images-and-the-registry.md)*
